...
export default function History() {
  ...
  useEffect(() => {
    try { setHistory(JSON.parse(localStorage.getItem("ts_history") || "[]")); } catch {}
  }, []);

  const clearAll = () => {
    if (confirm("Clear all history?")) { localStorage.removeItem("ts_history"); setHistory([]); }
  };

  const filtered = filter === "all" ? history : history.filter(h => h.result?.verdict === filter);
        ...
        <div style={{ maxWidth:660, margin:"0 auto" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:28, flexWrap:"wrap", gap:12 }}>
            <div>
              ...
              <h1 style={{ fontFamily:"'Space Mono',monospace", fontSize:20, fontWeight:700,
                color:"#fff", letterSpacing:3, textTransform:"uppercase", marginTop:8 }}>📋 History</h1>
              <p style={{ color:"#444", fontSize:12, marginTop:4 }}>{history.length} checks total</p>
            </div>
            {history.length > 0 && (
              <button onClick={clearAll} style={{ padding:"8px 16px", background:"rgba(255,23,68,0.08)",
                border:"1px solid rgba(255,23,68,0.2)", borderRadius:8, color:"#ff4466", fontSize:12, cursor:"pointer" }}>
                🗑️ Clear All
              </button>
            )}
          </div>
              ...
              {["all","TRUE","FALSE","MISLEADING"].map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  style={{ padding:"6px 14px", borderRadius:20,
                    ...
                    color:filter===f?"#00aaff":"#444", fontSize:12, cursor:"pointer" }}>
              ...
              <div key={i} style={{ background:"rgba(255,255,255,0.02)",
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8, marginBottom:8 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <span style={{ background:`${vc?.color}20`, color:vc?.color, fontSize:10, padding:"2px 8px", borderRadius:10, fontWeight:700 }}>
                      {item.result?.confidence}%
                    </span>
                  </div>
                  <span style={{ color:"#333", fontSize:11, whiteSpace:"nowrap" }}>{fmt(item.date)}</span>
                </div>
                ...
                <p style={{ color:"#888", fontSize:13, lineHeight:1.5 }}>{item.result?.summary}</p>
              </div>
            );
          })}

          {filtered.length === 0 && history.length > 0 && (
            <div style={{ textAlign:"center", padding:"40px", color:"#444", fontSize:14 }}>No results for this filter</div>
          )}
        </div>
      </div>
    </>
